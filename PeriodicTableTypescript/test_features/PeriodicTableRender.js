...
const currentTable = {
  category : "category",
  colors : colorLibrary.category.colors,
  ...
  function displayElement(tablePosition) {
    ...
    function determineColor() {
      for (let index in colors) {
        // if (typeof color.classification.type == "string" && color.charAt(0) === "-" || color.charAt(1) =="."){
        //   color= parseFloat(color);
        // } 
        if (typeof elementCategory == "string") {
          if(elementCategory == colors[index].classification){
            return colors[index].btnType;
          }
        } 
        else if (elementCategory !== null && elementCategory <= colors[index+1].classification) {
          if(elementCategory == color.classification){
            return colors[index].btnType;
          }
          // return colors[color];
        }
        else if (elementCategory !== null && elementCategory <= colors[index].classification) {
          if(elementCategory == color.classification){
            return colors[index].btnType;
          }
          // return colors[color];
...
function displayLegend() {
  ...
  let legendOutput = `<h4>${colorLibrary[currentTable.category].title}</h4>\n
  <table class = "legend">
  <tr>\n<td class= "align-top">\n<ul>\n`;
  var count = 0;
  for (color of colors) {
    ...
    legendOutput += `<li class="legenditem" id ="btn-${color.btnType}">\n
      <div class ="colorBox btn-${color.btnType}"></div>${color.classification}</li>\n`;
    count++;
  }
  return legendOutput;
}
