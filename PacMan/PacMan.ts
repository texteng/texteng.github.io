class World{
    // monsterlocations: { x: number; y: number; direction: string; strategy: string; }[];
    monsterlocations: any[];
    monsterlocationtable: number[][];
    standardsize: number;
    worldRowLength: number;
    numberofedibles: number;
    tiles: string[];
    pointValues: {
        1: number; // Coin 
        3: number; // Brick
...
class Monster{
    ...
    direction: string;
    strategy: string;
    constructor(_number: string, x: number, y: number, direction: string, strategy: string){
        this.number = _number;
        this.x = x; this.y = y;
