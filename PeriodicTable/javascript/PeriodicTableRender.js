        ...
        for (var column=0; column < rowlength; column++){
            ...
            switch(true) {
                ...
                case (tableposition == "RL"): //Row Label
                    table_output +=`<div id ="R${row}" class= "rowlabel boxsize h2">${row}</div>\n`;
                break;
...
function displayElement(atomicNumber, row, column){
    ...
    element_output += `<h6 class = "atomicnumber">${currentelement["number"]}</h6>\n`;
    element_output += `<h3 class = "elementsymbol">${currentelement["symbol"]}</h3>\n`;
    
    if(currentcategory == "groupBlock" || currentcategory == "category"){
        element_output += `<h6>${displayAtomicMass(currentelement["atomic_mass"])}</h6>\n`;
        element_output += `<h6 class = "elementname">${currentelement["name"]}</h6>\n`;
    }
    else{
        element_output += `<h6 class = "characterisitc">${currentelement[currentcategory]}</h6>\n`
    }
    ...
    function determineColor(currentelement){
        ...
        for(let index in currentDict){
            if (typeof currentelement[currentcategory] == "string"){
                if (currentelement[currentcategory].includes("unknown")||currentelement[currentcategory] == ""){
                    return "light";
                }
...
function elementInformationTitle(ElementNumber){
    ...
    elementinfomrationtitle_output += `${currentelement['name']} (${currentelement['symbol']}) `;
...
function elementInformation(ElementNumber){
    ...
    elementinformation_output += `<br><span class= 'font-weight-bold'>For More information see </span><a href="${currentelement['source']}">${currentelement['source']}</a>`
